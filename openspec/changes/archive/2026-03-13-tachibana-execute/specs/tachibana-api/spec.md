...
システムは SHALL sUrlEventWebSocket に WebSocket 接続し、約定通知をリアルタイムで受信する。

#### Scenario: WebSocket 接続と約定通知受信
- **WHEN** WebSocket に接続して約定通知を待機中に約定が成立した場合
- **THEN** 約定情報（銘柄、約定価格、約定数量）を含む通知を受信する
