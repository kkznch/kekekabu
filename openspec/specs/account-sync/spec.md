...
立花証券口座と kabu の DB を突合（リコンシリエーション）する機能。買付可能額・現物建玉を立花証券 API から取得し、DB の `portfolio_positions` との差分を検出する。`--fix` フラグで DB を実態に合わせて補正可能。配当・手数料・税金・入出金で発生する DB と実残高の乖離を可視化・修復する。
