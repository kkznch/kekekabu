...
- **THEN** 立花証券 API の `CLMZanKaiKanougaku` を仮想 URL（REQUEST）に圧縮 POST し、レスポンスを展開して `BrokerBalance { cash_available: String }` を返す

#### Scenario: 現物保有銘柄の取得
- **WHEN** `BrokerClient::query_positions()` を呼び出した場合
- **THEN** 立花証券 API の `CLMGenbutuKabuList` を仮想 URL（REQUEST）に圧縮 POST し、レスポンスを展開して `Vec<BrokerPosition>`（各要素は ticker, quantity, avg_cost）を返す。`aGenbutuKabuList` が空文字列の場合は空 Vec を返す
